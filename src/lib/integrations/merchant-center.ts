 ...
 *
 * Feed health (suspension / disapprovals) and the product catalogue run on the
 * Content API for Shopping v2.1, which needs only the `content` OAuth scope.
 * Price competitiveness runs on the newer Merchant API (Reports).
 *
 * Google sunsets Content API v2.1 on 2026-08-18. Its Merchant API replacements
 * additionally require the Cloud project to be *registered* with each Merchant
 * Center account (else a 401 "GCP project … not registered") — so the cutover is
 * gated on that registration and is deferred until it's in place. Merchant Center
 * IDs are discovered from the Google Ads `product_link` resource.
 *
 * Requires the `content` OAuth scope. Users who authenticated before this scope
 * was added may need to reconnect Google once.
 */
...
/**
 * Lists all products in the Merchant Center feed via Content API v2.1. Used as a
 * fallback when shopping_performance_view returns no rows (PMax campaigns serving
 * on non-Shopping surfaces). Uses the `content` scope — no GCP registration.
 *
 * NOTE: Content API v2.1 sunsets 2026-08-18; cut over to the Merchant API
 * Products sub-API before then, once the Cloud project is registered with the
 * Merchant Center account (same GCP-registration requirement as the health call).
 */
